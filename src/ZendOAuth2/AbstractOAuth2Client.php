...
abstract class AbstractOAuth2Client
{
    ...
    public function getInfo()
    {
        if(is_object($this->session->info)) {
            ...
            $fields = !is_null($this->options->getScopeFields()) ? 'fields=' . implode(',',$this->options->getScopeFields()) . '&' : '';

            $urlProfile = $this->options->getInfoUri() . '?' .$fields . 'access_token='.$this->session->token->access_token;
